...
class ImageEncoderOriginalNew(nn.Module):
    """
    ImageEncoder used in paper Neural Template
    Keep it here for pre-trained original weights
    ...
    """
    def __init__(self, config):
        super(ImageEncoderOriginalNew, self).__init__()
        self.img_ef_dim = config.img_ef_dim
        # TODO: Remove or refactor below
        """
        self.z_dim = config.decoder_input_embbeding_size * 2 if config.decoder_type == 'Flow' and hasattr(config,
...
class ResNet18(nn.Module):
    ...
    def forward(self, input):
        output = self.bn_initial(input)
        output = self.conv_0(output)
        output = self.bn0(output)
        output = F.leaky_relu(output, negative_slope=0.2, inplace=True)

        output = F.max_pool2d(output, (3,3), stride=2, padding=2)

        output = self.res_1(output)
        output = self.res_2(output)

        output = self.res_3(output)
        output = self.res_4(output)

        output = self.res_5(output)
        output = self.res_6(output)

        output = self.res_7(output)
...
class ResNet34(nn.Module):
    ...
    def forward(self, input):
        ...
        output = self.resnet_block_1(output)
        output = self.resnet_block_2(output)
        output = self.resnet_block_3(output)
        output = self.resnet_block_4(output)
...
class ResNet50(nn.Module):
    def __init__(self, in_f, base_f, out_f):
        ...
        self.bn_initial = nn.BatchNorm2d(in_f)
        self.conv_0 = nn.Conv2d(in_f, base_f, 3, stride=2, padding=2, bias=False)
        self.bn0 = nn.BatchNorm2d(base_f)
